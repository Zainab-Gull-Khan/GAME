...
var vine_scene = preload("res://scenes/vine_pair.tscn")
...
func _on_pipe_timer_timeout():
	...
	vine.position.x = screen_width + 30  # Add extra space so it appears off-screen
	vine.GAP = GAP
	vine.position.y = randf_range(prevY - VINE_OFFSET, prevY + VINE_OFFSET)
	if vine.getLowerY() > screen_height - 20:
		vine.position.y = screen_height - 20 - GAP
	if vine.getUpperY() < 20:
		vine.position.y = 20 + GAP
	
	prevY = vine.position.y
...
func _on_challenge_timer_timeout():
	GAP -= 20
	VINE_OFFSET += 5
	$vineTimer.wait_time -= 0.3
	clamp($vineTimer.wait_time, 0.75, 2.5)
