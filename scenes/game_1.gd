	...
	for fruit_name in ["apple", "banana", "orange", "coconut"]:
		...
		match fruit_name:
			"apple":
				...
				fruit.textures = Globals.textures[Globals.FruitType.APPLE]["whole"]
			"banana":
				fruit.fruit_type = Globals.FruitType.BANANA
				fruit.textures = Globals.textures[Globals.FruitType.BANANA]["whole"]
			"orange":
				fruit.fruit_type = Globals.FruitType.ORANGE
				fruit.textures = Globals.textures[Globals.FruitType.ORANGE]["whole"]
			"coconut":
				fruit.fruit_type = Globals.FruitType.COCONUT
				fruit.textures = Globals.textures[Globals.FruitType.COCONUT]["whole"]
...
func spawn_fruit():
	...
	for i in range(num_fruits):
		...
		fruit.velocity = Vector2(randf_range(INITIAL_X_VELOCITY_RANGE.x, INITIAL_X_VELOCITY_RANGE.y),
			INITIAL_Y_VELOCITY)
